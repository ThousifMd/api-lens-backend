-- Quota Management Additional Schema
-- This file contains additional tables needed for the quota management system

-- ============================================================================
-- QUOTA MANAGEMENT TABLES
-- ============================================================================

-- Quota configurations table (enhanced version)
...
-- ============================================================================
-- DEFAULT DATA FOR QUOTA MANAGEMENT
-- ============================================================================
...
SELECT 
    ...
    qc.auto_block,
    rlc.tier,
    -- Recent usage could be calculated via Redis or aggregated from cost_calculations
...
LEFT JOIN rate_limit_configs rlc ON c.id = rlc.company_id
LEFT JOIN (
    -- Subquery to get current month usage (this would be more accurate with Redis data)
    SELECT 
        company_id,
...
COMMENT ON TABLE quota_configurations IS 'Company-specific quota limits and settings';
COMMENT ON TABLE quota_alerts IS 'Quota alert history and notifications';
COMMENT ON TABLE quota_resets IS 'Audit trail for quota resets and modifications';
COMMENT ON TABLE quota_violations IS 'Record of quota violations for compliance and analytics';
COMMENT ON TABLE usage_analytics IS 'Aggregated usage analytics for reporting and insights';

COMMENT ON VIEW quota_status_overview IS 'Real-time overview of quota status for all companies';
COMMENT ON VIEW quota_violations_summary IS 'Summary of quota violations in the last 30 days';
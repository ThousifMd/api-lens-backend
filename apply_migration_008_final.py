#!/usr/bin/env python3
"""
Apply migration 008 - Add image generation support (Final version with slug)
"""
import asyncio
import os
...
async def apply_migration_008_final():
    ...
    try:
        ...
        print("\n🚀 Applying migration 008_add_image_generation_support (Final)...")
        print("=" * 80)
        ...
        async with conn.transaction():
            ...
            # 5. Update existing models to image_generation type and add new ones
            models_to_add = []
            
            # OpenAI models
            if 'openai' in vendor_map:
                models_to_add.extend([
                    (vendor_map['openai'], 'dall-e-3', 'dall-e-3', 'DALL-E 3', 'Most advanced image generation from OpenAI', 'image_generation'),
                    (vendor_map['openai'], 'dall-e-2', 'dall-e-2', 'DALL-E 2', 'High-quality image generation from OpenAI', 'image_generation')
                ])
            
            # Stability AI models - update existing ones
            if 'stability-ai' in vendor_map:
                # Update existing models to image_generation type
                await conn.execute("""
                ...
                # Add any missing Stability AI models
                models_to_add.append(
                    (vendor_map['stability-ai'], 'stable-diffusion-xl-beta-v2-2-2', 'stable-diffusion-xl-beta-v2-2-2', 
                     'SDXL Beta', 'Stable Diffusion XL Beta - Latest experimental version', 'image_generation')
                )
            
            # Adobe models - update existing ones
            if 'adobe' in vendor_map:
                # Update existing Adobe models to image_generation type
                await conn.execute("""
                    UPDATE vendor_models 
                    SET model_type = 'image_generation',
                        description = CASE 
                ...
                models_to_add.append(
                    (vendor_map['adobe'], 'firefly-v1', 'firefly-v1', 'Firefly v1', 
                     'Adobe Firefly v1 - Creative image generation', 'image_generation')
                )
            ...
            print(f"   ✅ Added {models_added} new models and updated existing ones")
        ...
        pricing = await conn.fetch("""
            ...
            JOIN vendors v ON vm.vendor_id = v.id
            WHERE vm.model_type = 'image_generation' AND vp.per_image_price IS NOT NULL
            ORDER BY v.name, vm.name;
        """)
        
        print(f"\n✅ Image generation pricing ({len(pricing)} models):")
        ...
        print("\n💡 Next steps:")
        print("   1. The image generation columns are now available")
        print("   2. Image generation models have been added/updated")
        print("   3. Pricing has been configured for per-image billing")
...
if __name__ == "__main__":
    asyncio.run(apply_migration_008_final())